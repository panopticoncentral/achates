...
- `CompletionWorkbookContent` retains original XLSX bytes, filename, SHA-256 ID, and a bounded preview for persistence/client history. `MessageConversion` replaces it with plain preview text before provider calls; workbook bytes never use the provider's binary file path.
