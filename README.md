...
- Interrupted replies show a notice and **Continue**, which resumes from saved tool results without rewinding the prompt or resending attachments. Timeout notices survive reopening the conversation. Active model output (including nested thinking calls) extends the idle deadline; defaults are five minutes without progress and a one-hour overall limit, configurable under `interactive` in `config.yaml`.
...
- Attach Excel `.xlsx` workbooks with **Choose Document** or drag and drop (8 MB each, up to four total attachments per message). The agent receives sheet dimensions and a small cell preview, then uses the built-in `workbook` tool to read sheets or cell ranges. Native model file support is unnecessary; range reads use tool calling. Values, formulas, cached formula results, and number formats are available; saved results may be stale or missing and are never recalculated. Numeric dates retain their Excel serial value and workbook date system. Charts, macros, external data refresh, and workbook editing are not supported.
- Uploaded workbooks are retained with the conversation, including after history compaction or resubmission, and removed when the conversation is deleted. The reader limits workbooks to 128 sheets, 250,000 stored cells, and 64 MB expanded content (16 MB per archive part). Reads default to `A1:J50` and accept ranges of up to 500 cells; oversized output asks the agent for a smaller range, and shortened cell text is explicitly marked.
