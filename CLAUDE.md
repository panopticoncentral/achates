...
- `ChatTool` — inter-agent communication. Actions: agents (list available agents with descriptions and tools), chat (start a ping-pong conversation with another agent). Per-session; requires `chat` in agent's tools list. Wired in `MobileTransport.CreateRuntime` (gated by `AgentDefinition.ToolNames` containing `chat`, since `ResolveTools` strips it from `Tools`); the `AgentInfo` registry is built per-call from the live `_agents` map so reloads/renames are reflected. Creates isolated `AgentRuntime` instances for both agents (target gets all its tools except chat to prevent cascade). Supports up to 15 back-and-forth turns; either agent can end early with `<<DONE>>`. `allow_chat` in agent config restricts which agents can be contacted (null/empty = all). Costs recorded to each agent's ledger with channel `chat`. **The caller (A)** keeps the transcript embedded as the tool result in its own session. **The target (B)** also gets its own persisted session (`Title = "Chat with {A}"`, `Source = SessionSource.Chat`, B's runtime messages) saved via the injected `MobileSessionStore`, and a `session.updated` broadcast — so B's nightly dreamtime can review the conversation and remember anything worthwhile instead of discarding it.
...
- `MobileSession` — session model with Id, Title, Created, Updated, Messages, optional `JobId` (cron-origin), and optional `Source` (`SessionSource?`; `Chat` marks an inter-agent-chat session recorded for the target agent, null = normal user/cron). `SessionSource` serializes as a snake_case string via `JsonStringEnumConverter`.
