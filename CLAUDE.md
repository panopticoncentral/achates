...
- **Read/unread tracking** — coarse-grained, **per-agent** (no per-message read receipts, no per-session counts). A single watermark per agent persists at `~/.achates/agents/{agentName}/read-state.json` as `{ "last_read_timestamp": <ms> }`. The `chat.read` RPC (params `{ agent, timestamp }`) advances the watermark **forward only**. `GetUnreadCountAsync` computes unread as the number of `AssistantMessage`s across all the agent's sessions newer than the watermark (walks sessions newest-first via `ListAsync`, short-circuiting once a session is entirely older); user messages don't count. The count surfaces as `unread_count` per agent in `agents.list`, served from `AgentStateCache` — an in-memory `ConcurrentDictionary` of `AgentPreviewState(LastMessage, LastActivity, UnreadCount)` so the list doesn't rescan every session file per call. `AgentStateCache.MarkRead` optimistically zeroes the cached count via a compare-and-swap retry loop; mutations invalidate entries, which are recomputed lazily by `ComputeAndCachePreviewAsync`.
