...
- RPC methods: `connect`, `ping`, `agents.list`, `sessions.list`, `sessions.create`, `sessions.get`, `sessions.delete`, `sessions.rename`, `sessions.delete_all`, `chat.send`, `chat.cancel`, `chat.read`, `agent.get`, `agent.update`, `agent.rename`, `agent.delete`, `agent.generate_avatar`, `tools.list`, `models.list`, `costs.summary`, `memory.list`, `memory.get`, `memory.set`, `jobs.list`, `jobs.update`, `jobs.delete`. `agent.delete` removes the agent's directory (sessions, memory, costs, cron, avatar, images), strips it from other agents' `allowed_chats`, and scaffolds a default agent if it was the last one.
