...
- `MemoryTool` — layered persistent memory with two scopes and two tiers within the agent scope. **Shared memory** at `~/.achates/memory.md` stores universal user facts (name, family, preferences) accessible to all agents. **Agent memory** has two tiers: the **core file** at `~/.achates/agents/{agentName}/memory.md` (always loaded into context) and the **archive directory** at `~/.achates/agents/{agentName}/memory/` (topical `.md` files retrieved on demand). Actions on the core and shared files: `read`, `save` (replaces the whole file), `append` (adds text to the end), `edit` (find-and-replace a unique substring; empty replacement deletes). Archive-specific actions (agent scope only): `list` (returns filenames + first-line summaries of all archive files) and `search` (keyword search across archive file contents). The `file` parameter routes `read`/`save`/`append`/`edit` to `memory/{file}.md` in the archive (path-guarded — must be a plain filename, no directory traversal); omitting `file` targets the core or shared file as usual. `append`/`edit` keep write payloads small so routine updates don't regenerate the entire file as one large tool call — this matters for big memories where a full `save` is slow and stall-prone (the dreamtime prompt nudges toward incremental edits). `scope` parameter (`shared` or `agent`) controls which tier to target; `read` without a scope returns both shared and agent core (not the archive). Survives session boundaries. **Soft budget**: when the agent-scope core is over budget, the tool appends a non-blocking nudge to consolidate content into the archive — it never rejects the write. The nudge surfaces on agent-scope core `read`, `save`, `append`, and `edit` (not shared, not archive); surfacing it on `read`/`edit` — not just `save`/`append` — matters because dreamtime reads core once up front and maintains it via `edit`, so a write-only nudge was never seen and consolidation silently never ran. Budget resolved per-agent: `**Memory Budget:**` in AGENT.md → `memory.default_budget_tokens` in config → hardcoded fallback of 8 000 tokens. Per-agent opt-out via the `**Shared Memory:** false` capability in AGENT.md (default `true`) — when off, the tool's schema omits the `scope` parameter entirely and the model only sees the agent-local core file. This is the roleplay-friendly mode: it prevents real-world identity facts from polluting in-character context.
...
tools:
  ...
  library:
    root: ~/path/to/library   # read-only reference documents (.md, text, .pdf)
  web_search:
...
memory:
  default_budget_tokens: 8000   # soft budget for agents' core memory; per-agent **Memory Budget:** overrides

cron:
  keep_last_per_job: 1    # sessions retained per cron job (default 1, 0 to disable)
  max_age_days: 7         # absolute ceiling on cron session age (default 7)
```
...
Capabilities keys: `Provider`, `Model`, `Thinking Model`, `Tools`, `Allowed Chats`, `Reasoning Effort`, `Temperature`, `Max Tokens`, `Dreamtime`, `Shared Memory`, `Voice`, `Speech Rate`, `Memory Budget`. List values use sub-bullets; scalar values go inline after the key. `Model` and `Thinking Model` are optional — when omitted, the agent uses `models.base` / `models.thinking` from `config.yaml` as a fallback. `Shared Memory` defaults to `true` when omitted; set to `false` for agents (typically roleplay/in-character) that should not see the shared user memory file. `Voice` is the per-agent TTS voice id (a Kokoro voice like `af_nicole` or a blend like `af_nicole(0.7)+af_bella(0.3)`); when omitted, the agent is voiceless unless `tools.speech.default_voice` is set globally. `Speech Rate` is the per-agent TTS rate (Kokoro's `speed`); accepts `[0.25, 4.0]`, clamped on parse; omitted means "use Kokoro's default (1.0)" and the field is dropped from the request body entirely. `Memory Budget` is the per-agent soft budget (in tokens) for the core memory file; when omitted, falls back to `memory.default_budget_tokens` in config.yaml, then to the hardcoded default of 8 000 tokens. Exceeding the budget triggers a non-blocking consolidation nudge on agent-scope core reads and writes (`read`/`save`/`append`/`edit`) — writes are never rejected.
...
~/.achates/agents/{agentName}/memory.md                        Agent memory — core file (always in context)
~/.achates/agents/{agentName}/memory/                          Agent memory — archive (topical .md files, retrieved on demand)
...
~/.achates/agents/{agentName}/read-state.json                  Read tracking (per-session watermarks + legacy floor)
