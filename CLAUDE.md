...
- `HealthTool` — queries health data from Withings API. Actions: weight (body composition), blood_pressure, sleep (summary by default; `detail: "segments"` for raw stage timeline), activity, workouts (discrete sessions: run, cycle, swim, etc.), authorize. Singleton; requires `withings` config with client_id and client_secret. OAuth 2.0 authorization code flow with browser redirect to `/withings/callback`.
