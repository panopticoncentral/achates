...
- `CostTool` — queries the persistent cost ledgers across one or all agents. Actions: `summary` (totals for a period), `recent` (last N entries), `breakdown` (grouped by `day`, `model`, `agent`, `channel`, or `peer`). New `scope` parameter (default `"self"`) accepts `"self"` (calling agent — backwards-compatible default), `"all"` (aggregate across every agent — for an uberagent overseeing total spend), or a specific agent name; unknown agent names produce a friendly error listing the available ones. Output also surfaces `channel` (direct turn vs `chat` vs `cron`), `peer` (initiator agent / job id), cache-write tokens, and a per-category cost split when those fields carry data. Per-session; requires `cost` in agent's tools list. Built with a snapshot of every agent's ledger in `MobileTransport.CreateRuntime` and `CronService.BuildJobTools` / `BuildDreamtimeTools` (rebuilt per call so reloads/renames take effect on next session). Ledger writes are always recorded regardless of tool config.
...
- `MobileSession` — session model with Id, Title, Created, Updated, Messages, optional `JobId` (cron-origin), and optional `Source` (`SessionSource?`; `Chat` marks an inter-agent-chat session recorded for the target agent, null = normal user/cron). For chat-origin sessions, `OriginSessionId` (the initiator's session id) and `PeerAgentId` (the initiator agent) track the pairing so the same continuing session is reused across rounds. `SessionSource` serializes as a snake_case string via `JsonStringEnumConverter`.
...
- Broadcast events: `session.updated`, `agents.changed`, `agent.renamed`, `cron.result`, `memory.updated`, `jobs.updated`, plus agent streaming events (`text.delta`, `text.end`, `thinking.delta`, `thinking.end`, `tool.start`, `tool.end`, `image.block`, `agent_turn.start`, `agent_turn.delta`, `agent_turn.end`, `message.end`, `done`). The `agent_turn.*` trio streams a consulted agent's reply live during `chat`/`ask` (first-class attributed bubbles, not a collapsed tool chip).
